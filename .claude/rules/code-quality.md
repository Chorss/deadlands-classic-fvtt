...
This is enforced by `tools/audit-css.mjs` (run via `/verify-system`, pre-commit hook on `.hbs`/`.css`/`.mjs` changes, and CI). A commit that adds a template class without a CSS rule is rejected. Classes built in `module/**/*.mjs` template literals are scanned too and held to the same rule — a commit that adds one without a CSS rule is rejected (M6 closed the last excused backlog entry, `dlc-hand-dialog`; `MODULE_BACKLOG` in the tool is now an empty, documented set). It also lists selectors defined in `styles/` that nothing uses, split into genuinely dead vs. probably reached through a dynamic fragment — informational only, never a gate. A class named only in a CSS comment does **not** count as defined.
